...
namespace backend\widgets\froala\editor\src;
...
class FroalaEditorWidget extends InputWidget
{
    ...
    public function registerClientScript()
    {
        ...
        if ($themeType != 'default') {
            $view->registerCssFile("{$asset->baseUrl}/css/themes/{$themeType}.css", ['depends' => FroalaEditorAsset::class]);
        }
        //language
        $langType = isset($this->clientOptions['language']) ? $this->clientOptions['language'] : 'en_gb';
        if ($langType != 'es_gb') {
            $view->registerJsFile("{$asset->baseUrl}/js/languages/{$langType}.js", ['depends' => FroalaEditorAsset::class]);
        }
